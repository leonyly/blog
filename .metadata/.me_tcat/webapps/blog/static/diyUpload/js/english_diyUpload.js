    ...
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadError', function (file, reason) {
                ...
                var err = 'Upload failed! File:' + file.name + ' Code:' + reason;
                if (errorCallBack) {
                    errorCallBack(err);
                }
            });
            ...
            webUploader.on('error', function (code) {
                ...
                switch (code) {
                    case  'F_DUPLICATE' :
                        text = 'You have choosed this file!';
                        break;
                    case  'Q_EXCEED_NUM_LIMIT' :
                        text = 'File nums length not allowed!';
                        break;
                    case  'F_EXCEED_SIZE' :
                        text = 'The file is too big!';
                        break;
                    case  'Q_EXCEED_SIZE_LIMIT' :
                        text = 'All files size are too big!';
                        break;
                    case 'Q_TYPE_DENIED' :
                        text = 'File type error!';
                        break;
                    default :
                        text = 'Unknow exception!';
                        break;
                }
                alert(text);
            });
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        if ($parentFileBox.find('.diyButton').length <= 0) {
            ...
            var div = '<div class="diyButton"> \
						<a class="diyStart" href="javascript:void(0)">Start</a> \
						<a class="diyCancelAll" href="javascript:void(0)">Cancel</a> \
					</div>';
            ...
            var uploadStart = function () {
                ...
                $startButton.text('Pause').one('click', function () {
                    webUploader.stop();
                    $(this).text('Go on').one('click', function () {
                        uploadStart();
                    });
                });
            }
